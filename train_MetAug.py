...
def parse_option():
    parser = argparse.ArgumentParser('metaug')
...
def train(epoch, train_loader, model, contrast, criterion_gh, optimizer, l_mtgen, l_mtgen_op, ab_mtgen, ab_mtgen_op, ori_mtgen, ori_mtgen_op, opt):
    ...
    for idx, (inputs, _, index) in enumerate(train_loader):
        ...
        mt_feat_l = l_mtgen(feat_l)
        mt_feat_ab = ab_mtgen(feat_ab)
        mt_feat_ori = ori_mtgen(feat_ori)
        # ori loss calc
...
def main():
    ...
    l_mtgen_op = torch.optim.SGD(l_mtgen.parameters(), lr=args.mt_learning_rate, momentum=args.mt_momentum, weight_decay=args.mt_weight_decay)
    ab_mtgen_op = torch.optim.SGD(ab_mtgen.parameters(), lr=args.mt_learning_rate, momentum=args.mt_momentum, weight_decay=args.mt_weight_decay)
    ori_mtgen_op = torch.optim.SGD(ori_mtgen.parameters(), lr=args.mt_learning_rate, momentum=args.mt_momentum, weight_decay=args.mt_weight_decay)
    # set mixed precision
    if args.amp:
